...
SOURCE_FILES = [
    (os.path.join(ROOT, "src", "Installer", "Program.cs"),
     ...
     r'\g<1>{VERSION}\2'),
    (os.path.join(ROOT, "src", "Mod", "RussianLocPlugin.cs"),
     r'(BepInPlugin\("com\.nottonight\.russianlocalization", "Not Tonight Russian", ")[^"]+(")',
     r'\g<1>{VERSION}\2'),
    (os.path.join(ROOT, "src", "Mod", "RussianLocPlugin.cs"),
     r'(Not Tonight Russian Localization plugin loaded v)[0-9]+\.[0-9]+\.[0-9]+',
     r'\g<1>{VERSION}'),
]
